var mailformat = /^(?:[a-zA-Z0-9!#$%&'*+=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&'*+=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-zA-Z0-9-]*[a-zA-Z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])$/;


var phoneFormat = /^(\d{3}[-\s]?){2}\d{4}$/;


function isValidEmail(email) {
   
    return mailformat.test(email);
}


//function isValidPhoneNumber(phone) {

//    return phoneFormat.test(phone);
//}

function isValidPhoneNumber(phone) {
   

    if (!phoneFormat.test(phone)) {
        return false; // Not a valid format
    }

 
    var digitsOnly = phone.replace(/[-\s]/g, '');


    if (/^(\d)\1*$/.test(digitsOnly)) {
        return false; // All digits are the same, not valid
    }

    // Count the number of unique digits in the phone number
    var uniqueDigits = new Set(digitsOnly).size;

    // Require at least 3 unique digits for a valid phone number
    return uniqueDigits >= 3;
}

